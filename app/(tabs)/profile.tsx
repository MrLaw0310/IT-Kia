...
const ANNUAL_FEE   = 10; // RM per vehicle
    ...
    <View style={styles.vehicleCard}>
      <View style={styles.vehicleLeft}>
        ...
        <View>
          ...
          <View style={[styles.paidBadge, { backgroundColor: vehicle.isPaid ? C.green + "18" : C.red + "18", borderColor: vehicle.isPaid ? C.green + "44" : C.red + "44" }]}>
      ...
      <View style={styles.vehicleActions}>
        <TouchableOpacity onPress={onEdit} style={styles.vActionBtn}>
          <Text style={styles.vActionEdit}>Edit</Text>
        </TouchableOpacity>
        <TouchableOpacity onPress={onRemove} style={styles.vActionBtn}>
          <Text style={styles.vActionRemove}>Remove</Text>
        </TouchableOpacity>
      </View>
    </View>
  );
...
export default function ProfileScreen() {
  ...
  const [notifParking, setNotifP]       = useState(true);
  const [notifOverstay, setNotifO]      = useState(true);
  ...
  function addVehicle(plate: string, model: string, isOKU: boolean) {
    ...
    Alert.alert("✅ Registered", `${plate} added. Please proceed to pay RM${ANNUAL_FEE} at the admin office.`);
  }
    ...
    <View style={styles.screen}>
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scroll}>
        ...
        <View style={styles.sectionHeader}>
          ...
          <Text style={styles.sectionSub}>{vehicles.length}/{MAX_VEHICLES} registered · RM{ANNUAL_FEE}/vehicle/year</Text>
        </View>
        ...
        {vehicles.map(v => (
          <VehicleCard
            key={v.id}
            vehicle={v}
            onEdit={() => { setEditTarget(v); setModalVisible(true); }}
        ...
        <View style={styles.settingCard}>
          <SettingRow
            ...
            right={
              <Switch value={notifParking} onValueChange={setNotifP}
                trackColor={{ false: C.border, true: C.accent + "80" }}
                thumbColor={notifParking ? C.accent : C.muted} />
            }
          />
          ...
          <SettingRow
            ...
            right={
              <Switch value={notifOverstay} onValueChange={setNotifO}
                trackColor={{ false: C.border, true: C.accent + "80" }}
                thumbColor={notifOverstay ? C.accent : C.muted} />
            }
          />
        </View>
        ...
        <View style={styles.settingCard}>
          ...
          <SettingRow icon="📷" label="Camera Permission"
            sub="Used for plate scanning at check-in"
            onPress={() => Alert.alert("Camera", "Go to phone Settings → Apps → MDIS Parking → Permissions.")} />
          <View style={styles.rowDivider} />
          <SettingRow icon="📍" label="Location Permission"
            sub="Used for GPS navigation to campus"
            onPress={() => Alert.alert("Location", "Go to phone Settings → Apps → MDIS Parking → Permissions.")} />
          <View style={styles.rowDivider} />
...
const styles = StyleSheet.create({
  ...
  vehicleActions: { gap: 6 },
  vActionBtn: { alignItems: "center" },
